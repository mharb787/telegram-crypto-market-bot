...
async function buildAssetAnalysis(asset, strategy, bitcoinState) {
  ...
  const dailyCloses = candlesDaily.map((c) => c.close);
  const dailyEma50 = ema(dailyCloses, 50);
  const dailyEma50TenAgo = ema(dailyCloses.slice(0, -10), 50);
  const aboveDailyEma50 = dailyEma50 !== null ? current > dailyEma50 : true;
  const ema50Rising = dailyEma50 !== null && dailyEma50TenAgo !== null ? dailyEma50 > dailyEma50TenAgo : true;
  const rawConfidence = clamp(Math.round((weighted / totalWeight) * 100 * sessionMultiplier), 0, 100);
  const confidence = (aboveDailyEma50 && ema50Rising) ? rawConfidence : Math.min(rawConfidence, 42);
